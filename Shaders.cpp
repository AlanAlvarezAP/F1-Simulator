...
"   if(isLightCube){\n"
"       outColor = vec4(ouColor, 1.0);\n"
"       return;\n"
"   }\n"

    // 1. Componente Ambiental
	// Cambiar el 0.algo
"   vec3 ambient = Ka * lightColor *2.0;\n"

    // 2. Componente Difusa
"   float diff = max(dot(norm, lightDir), 0.0);\n"
"   vec3 diffuse = (diff * Kd) * lightColor * 1.0;\n"

    // 3. Componente Especular
"   vec3 viewDir = normalize(viewPos - FragPos);\n"
"   vec3 reflectDir = reflect(-lightDir, norm);\n"
"   float spec = pow(max(dot(viewDir, reflectDir), 0.0), Ns);\n"
"   vec3 specular = (spec * Ks) * lightColor *1.0;\n"
...
void Shaders::SetColor(const float &r,const float &g,const float &b){
	int Color_loc=glGetUniformLocation(this->program_id,"ouColor");
...
void Shaders::SetIsLightCube(bool value){
    int loc = glGetUniformLocation(program_id, "isLightCube");
    
    if(loc == -1){
        // Puedes comentar el cout después de verificar que funciona para evitar spam
        // std::cout << "Couldn't find isLightCube\n"; 
        return;
    }
    glUniform1i(loc, value);
}
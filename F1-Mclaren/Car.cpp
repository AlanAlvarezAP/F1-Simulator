...
void Llanta::DrawGeometry(const Matrix& parent){
    ...
    // Configuramos la luz
    float lightPos[] = { 10.0f, 20.0f, 10.0f }; // Luz estática arriba del carro
    float lightColor[] = { 1.0f, 1.0f, 1.0f };  // Luz blanca
    float camPos[] = { 0.0f, 5.0f, 10.0f };     // Posición aproximada de la cámara
    Shader.SetLight(lightPos, lightColor, camPos);
...
void Car::Generate(){
	...
	while(std::getline(lector,line)){
		...
		if(inicio=='m'){
			...
			std::string path_mtl = "F:/LabCompiler2026_I_CG/glfw-master/OwnProjects/F1-Mclaren/models/" + mtlib;
			...
			if(new_mesh){
				...
				std::string path="F:/LabCompiler2026_I_CG/glfw-master/OwnProjects/F1-Mclaren/textures/";

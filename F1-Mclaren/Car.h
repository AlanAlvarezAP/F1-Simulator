...
public:
	...
	// for animation
	bool scriptControlsSteering = false;
	float targetSteerAngle = 0.0f;
	float currentSteerAngle = 0.0f;
	std::queue<ScriptStep> animScript;
    float currentStepTimer = 0.0f;
    bool isScriptPlaying = false;
public:
	...
	// for animation
	void addScriptStep(bool fwd, bool brk, float duration, float speed, float angle);
	void updateScriptAnimation(float dt, bool& outFwd, bool& outBrk);
};